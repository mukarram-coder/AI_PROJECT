...
class Ui_AISearchingTechniquesMainWindow(object):
        ...
        with open('test.txt', 'w') as f:
            ...
            if graphType == "Undirectd Graph":
                if searchType == "BFS":
                    ...
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path4, goal = graphiterative.iterative_deepening_search(start, goals)
                    if traced_path4:
                        print('Path:', end=' ')
                        Graph_iterative.print_path(traced_path4, goal)
                        print()
                    ...
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path6, cost4, goal = self.graphbestfirst.best_first_search(start, goals)
                    if traced_path6:
                        print('Path:', end=' ')
                    ...
                    graph = nx.Graph()
                    for i in range(0, self.counter):
                        graph.add_edge(Node1_arr[i], Node2_arr[i], weight=int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goal = Goal_list[0]
                    traced_path7, cost5 = self.simulatedannealing.simulated_annealing_search(graph, start, goal)
                    if traced_path7:
                        print('Path:', end=' ')
                        print(traced_path7)
                        print('Cost:', cost5)
    ...
    def setupUi(self, AISearchingTechniquesMainWindow):
        ...
        self.AlphaBetaButton = QtWidgets.QPushButton(self.centralwidget)
        self.AlphaBetaButton.setGeometry(QtCore.QRect(630, 340, 131, 31))
        self.AlphaBetaButton.setObjectName("AlphaBetaButton")
        self.AlphaBetaButton.clicked.connect(self.AlphaBetaClicked)

        self.SimulatedAnnealingButton = QtWidgets.QPushButton(self.centralwidget)
        self.SimulatedAnnealingButton.setGeometry(QtCore.QRect(630, 400, 131, 31))
        self.SimulatedAnnealingButton.setObjectName("SimulatedAnnealingButton")
        self.SimulatedAnnealingButton.clicked.connect(self.SimulatedAnnealingClicked)
    ...
    def retranslateUi(self, AISearchingTechniquesMainWindow):
        ...
        self.AlphaBetaButton.setText(_translate("AISearchingTechniquesMainWindow", "Alpha-Beta Pruning"))
        self.SimulatedAnnealingButton.setText(_translate("AISearchingTechniquesMainWindow", "Simulated Annealing"))
